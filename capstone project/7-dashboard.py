...
spacex_df = pd.read_csv("data_dashboard.csv")
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(values=df.values, names=df.index, 
                     title='Percentage of successful launches by every site')
        return fig
    else:
        df = spacex_df[spacex_df['Launch Site'] == entered_site]['class'].value_counts().reset_index()
        df['classname'] = df['class'].map({0 : 'failure', 1 : 'success'})
        title = "Successful and failed launches from " + entered_site
        fig = px.pie(values=df['count'], names=df['classname'], title=title)
        return fig
